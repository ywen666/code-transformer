...
class Rank1PointerNetwork(nn.Module):

    def __init__(self, d_model, subtokens_per_token, pointer_attention_type: AttentionType, n_attention_heads=8, num_languages=4):
        super(Rank1PointerNetwork, self).__init__()
        ...
        self.linear1_alpha = nn.Parameter(torch.FloatTensor(num_languages, self.d_model))
        self.linear1_gamma = nn.Parameter(torch.FloatTensor(num_languages, self.d_model))
        self.linear2_alpha = nn.Parameter(torch.FloatTensor(num_languages, self.d_model))
        self.linear2_gamma = nn.Parameter(torch.FloatTensor(num_languages, subtokens_per_token * self.d_model))
    ...
    #         pointer_input_subtokens = transformer_output.all_emb[-1][0].transpose(0, 1)
    def init_batch(self, pointer_input_subtokens, pointer_pad_mask, extended_vocabulary_ids, len_vocab, languages):
        bsz = pointer_input_subtokens.shape[0]
        seq_len = pointer_input_subtokens.shape[1]
        device = next(self.parameters()).device
        ...
        alpha = torch.index_select(self.linear2_alpha, 0, languages).unsqueeze(1)
        gamma = torch.index_select(self.linear2_gamma, 0, languages).unsqueeze(1)
        perturbed_pointer_input_subtokens =  pointer_input_subtokens * alpha
        pointer_input_subtokens = self.subtoken_extractor_linear(perturbed_pointer_input_subtokens) * gamma
        pointer_input_subtokens = pointer_input_subtokens.view(bsz, seq_len, self.d_model, -1)
        pointer_input_subtokens = pointer_input_subtokens.transpose(2, 3)
        pointer_input_subtokens = [s[m] for s, m in zip(pointer_input_subtokens, pointer_pad_mask)]
        # bsz x seq_len_subtokens
        pointer_subtoken_mask = pad_sequence(
    ...
    def calculate_pointer_attention(self, pointer_query: torch.Tensor, languages: torch.Tensor):
        bsz = pointer_query.shape[0]
        device = next(self.parameters()).device

        # Use last decoder hidden state to produce query vector for pointer
        # bsz x d_model
        alpha = torch.index_select(self.linear1_alpha, 0, languages)
        gamma = torch.index_select(self.linear1_gamma, 0, languages)
        perturbed_pointer_query = pointer_query * alpha
        pointer_query = self.query_linear(pointer_query) * gamma
        pointer_query = torch.tanh(pointer_query)
        pointer_query = pointer_query.unsqueeze(-1)
